...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        return _clamp(ap_sum / n_relevant)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            "Return JSON mapping criterion names to numeric scores."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        overall_avg = sum(all_values) / len(all_values) if all_values else 0.5

        positional_bias = False
        if len(response_avgs) >= 2:
            rest_avg = sum(response_avgs[1:]) / len(response_avgs[1:])
            positional_bias = response_avgs[0] > rest_avg
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        regressions = [
            name
            for name, (new_avg, baseline_avg) in comparisons.items()
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Clarify system prompt and add intent routing for off-topic answers"
            )
        ...
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Improve intent detection so answers stay on the customer question"
            )

        defaults = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Improve retrieval ranking so relevant evidence appears earlier",
            "Augment the golden dataset with failing edge cases and re-run evaluation",
        ]
