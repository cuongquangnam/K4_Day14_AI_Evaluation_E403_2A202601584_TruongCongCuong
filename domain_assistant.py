...
class GeminiGenerator:
    """Generate answers with the Gemini API (google-genai SDK)."""
    ...
    def __init__(self, max_output_tokens: int = 300) -> None:
        ...
        if not api_key:
            raise RuntimeError(
                ...
                "(OPENAI_API_KEY is also accepted as a fallback name)"
            )
...
# Keep the old name as an alias so existing imports/docs still work.
